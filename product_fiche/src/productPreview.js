...
const MovieCard = ({id=1}) => {
  ...
  return (
    <div className="relative w-full h-screen flex items-center justify-center"> {/* Centrage du bouton Open */}
      {/* Afficher seulement le bouton "Open" si la carte est fermée */}
      {!showCard && (
        <button 
          onClick={() => setShowCard(true)} 
          className="flex items-center bg-blue-500 text-white px-6 py-2 font-bold rounded text-lg">
          Ouvrir
        </button>
      )}

      {/* Afficher la carte si showCard est vrai */}
      {showCard && (
